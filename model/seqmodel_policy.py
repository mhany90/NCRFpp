...
class SeqModel(nn.Module):
    def __init__(self, data):
        ...
        self.action_space = data.label_alphabet_size
    ...
    def sample(self, word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover, batch_label, mask, no_samples):
        ...
        for n in range(no_samples):
            ...
            score = F.log_softmax(outs, 1)
            #total_loss = loss_function(score, batch_label.view(batch_size * seq_len))
            tag_seq = torch.multinomial(score, num_samples=1)#.squeeze(1)
            #_, tag_seq  = torch.max(score, 1)
            tag_seq = tag_seq.view(batch_size, seq_len)
            scores = score.view(batch_size, seq_len)
            samples_tag_sequences.append(tag_seq)
            #if self.average_batch:
             #   total_loss = total_loss / batch_size
        return scores ,samples_tag_sequences
